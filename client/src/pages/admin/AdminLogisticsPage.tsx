              ...
              <div key={o._id} className="p-5 bg-slate-900/60 border border-slate-800 rounded-3xl space-y-4 hover:border-slate-700/80 transition-all">
                ...
                <div className="grid grid-cols-1 md:grid-cols-3 gap-4 text-xs">
                  ...
                  <div className="space-y-1 bg-slate-950/40 p-3 rounded-xl border border-slate-800/80">
                    <label className="text-slate-400 font-semibold block text-[10px] uppercase">Destination & Customer</label>
                    <p className="text-white font-bold truncate text-xs">{customerName} {customerPhone ? `(${customerPhone})` : ''}</p>
                    <p className="text-slate-300 text-[11px] truncate">{o.shippingAddress?.fullAddress || ''}</p>
                    <p className="text-slate-400 text-[10px]">{o.shippingAddress?.city}, {o.shippingAddress?.postalCode}</p>
                  </div>
                </div>
              </div>
            );
