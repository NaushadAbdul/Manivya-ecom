...
exports.deleteHero3DCategory = exports.updateHero3DCategory = exports.createHero3DCategory = exports.getAllHero3DCategoriesAdmin = exports.getHero3DCategories = exports.DEFAULT_HERO_3D_ITEMS = void 0;
const Hero3DCategory_1 = __importDefault(require("../models/Hero3DCategory"));
const apiResponse_1 = require("../utils/apiResponse");
exports.DEFAULT_HERO_3D_ITEMS = [
    {
        id: 'gelato',
        name: 'Gelato',
        watermark: 'GELATO',
...
const getHero3DCategories = async (_req, res) => {
    try {
        let items = await Hero3DCategory_1.default.find({ isActive: true }).sort({ sortOrder: 1 });
        if (!items || items.length === 0) {
            // Seed default items into DB if empty
            await Hero3DCategory_1.default.insertMany(exports.DEFAULT_HERO_3D_ITEMS);
            items = await Hero3DCategory_1.default.find({ isActive: true }).sort({ sortOrder: 1 });
        }
        return (0, apiResponse_1.sendSuccess)(res, items, '3D Hero categories retrieved');
    }
    catch (err) {
        return (0, apiResponse_1.sendError)(res, err.message, 500);
    }
};
exports.getHero3DCategories = getHero3DCategories;
// GET /api/hero-3d/admin (Admin - includes inactive)
const getAllHero3DCategoriesAdmin = async (req, res) => {
    try {
        if (!req.user || req.user.role !== 'admin') {
            return (0, apiResponse_1.sendError)(res, 'Admin access required', 403);
        }
        let items = await Hero3DCategory_1.default.find({}).sort({ sortOrder: 1 });
        if (!items || items.length === 0) {
            await Hero3DCategory_1.default.insertMany(exports.DEFAULT_HERO_3D_ITEMS);
            items = await Hero3DCategory_1.default.find({}).sort({ sortOrder: 1 });
        }
        return (0, apiResponse_1.sendSuccess)(res, items, 'All 3D Hero categories retrieved for Admin');
    }
    catch (err) {
        return (0, apiResponse_1.sendError)(res, err.message, 500);
    }
};
exports.getAllHero3DCategoriesAdmin = getAllHero3DCategoriesAdmin;
// POST /api/hero-3d (Admin Create)
const createHero3DCategory = async (req, res) => {
    try {
        if (!req.user || req.user.role !== 'admin') {
            return (0, apiResponse_1.sendError)(res, 'Admin access required', 403);
        }
        ...
        if (!itemData.name || !itemData.title || !itemData.image) {
            return (0, apiResponse_1.sendError)(res, 'Name, title, and image are required', 400);
        }
        const id = itemData.id || itemData.name.toLowerCase().replace(/[^a-z0-9]/g, '');
        const existing = await Hero3DCategory_1.default.findOne({ id });
        if (existing) {
            return (0, apiResponse_1.sendError)(res, 'Item with this identifier already exists', 400);
        }
        const newItem = new Hero3DCategory_1.default({
            ...itemData,
            id,
        });
        await newItem.save();
        return (0, apiResponse_1.sendSuccess)(res, newItem, '3D Hero category created successfully', 211);
    }
    catch (err) {
        return (0, apiResponse_1.sendError)(res, err.message, 500);
    }
};
exports.createHero3DCategory = createHero3DCategory;
// PUT /api/hero-3d/:id (Admin Edit)
const updateHero3DCategory = async (req, res) => {
    try {
        if (!req.user || req.user.role !== 'admin') {
            return (0, apiResponse_1.sendError)(res, 'Admin access required', 403);
        }
        const { id } = req.params;
        const item = await Hero3DCategory_1.default.findOne({ id });
        if (!item) {
            return (0, apiResponse_1.sendError)(res, '3D Hero category not found', 404);
        }
        Object.assign(item, req.body);
        await item.save();
        return (0, apiResponse_1.sendSuccess)(res, item, '3D Hero category updated successfully');
    }
    catch (err) {
        return (0, apiResponse_1.sendError)(res, err.message, 500);
    }
};
exports.updateHero3DCategory = updateHero3DCategory;
// DELETE /api/hero-3d/:id (Admin Delete)
const deleteHero3DCategory = async (req, res) => {
    try {
        if (!req.user || req.user.role !== 'admin') {
            return (0, apiResponse_1.sendError)(res, 'Admin access required', 403);
        }
        const { id } = req.params;
        const deleted = await Hero3DCategory_1.default.findOneAndDelete({ id });
        if (!deleted) {
            return (0, apiResponse_1.sendError)(res, '3D Hero category not found', 404);
        }
        return (0, apiResponse_1.sendSuccess)(res, deleted, '3D Hero category deleted successfully');
    }
    catch (err) {
        return (0, apiResponse_1.sendError)(res, err.message, 500);
    }
};
exports.deleteHero3DCategory = deleteHero3DCategory;
