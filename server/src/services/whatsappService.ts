 ...
 */
const normalisePhone = (phone: string): string | null => {
...
export const sendOrderWhatsApp = async (data: WhatsAppOrderData): Promise<void> => {
  ...
  const paymentLabel =
    data.paymentMethod === 'cod' ? '💵 Cash on Delivery' : '📲 QR Code Payment';

  const message =
    `🛍️ *MANIVYA – Order Confirmed!*\n` +
  ...
  try {
    ...
    console.error('❌ Failed to send WhatsApp message:', (err as Error).message);
  }
};
